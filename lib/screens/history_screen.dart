                              ...
                              child: Column(
                                ...
                                children: [
                                  Row(children: [
                                    ...
                                    VerificationStatusBadge(status: '${r['verification_status'] ?? 'pending'}'),
                                  ]),
...
class HistoryDetailPage extends StatelessWidget {
  ...
  List<String> _xaiLines(Map<String, dynamic> item) {
    ...
    if (limiting is List) lines.addAll(limiting.map((e) => 'Limiting factor: $e'));
        ...
        child: ListView(
          ...
          children: [
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        VerificationStatusBadge(status: '${item['verification_status'] ?? 'draft'}'),
                        if ('${item['verification_status'] ?? ''}' == 'rejected' && '${item['planner_notes'] ?? ''}'.trim().isNotEmpty) ...[
                          const SizedBox(height: 8),
                          Container(
                            width: double.infinity,
                            padding: const EdgeInsets.all(10),
                            decoration: BoxDecoration(color: const Color(0xFFFFF0F0), borderRadius: BorderRadius.circular(10)),
                            child: Text('Planner feedback: ${item['planner_notes']}', style: const TextStyle(fontSize: 12.5, color: Color(0xFF8A2C2C))),
                          ),
                        ],
                        const SizedBox(height: 6),
                        ...
                        if ('${item['analysis_summary'] ?? ''}'.trim().isNotEmpty) ...[
                          const Divider(height: 24),
                          const Text('ANALYSIS SUMMARY', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: green)),
                          const SizedBox(height: 7),
                          Text('${item['analysis_summary']}', style: const TextStyle(fontSize: 13, height: 1.45)),
                        ],
                        ...
                        DetailLine(label: 'Infrastructure Recommendation', value: '${item['infrastructure_recommendation'] ?? '--'}'),
                        DetailLine(label: 'Slope', value: '${state.numText(item['slope_pct'] ?? item['slope']) == '--' ? '--' : '${state.numText(item['slope_pct'] ?? item['slope'])}%'}'),
                      ],
                    ),
                  ),
                ],
                ...
                child: Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Row(children: [
                      ...
                      SizedBox(width: 8),
                      Expanded(child: SectionTitle('EXPLAINABLE AI — WHY THIS CROP?')),
                    ]),
                    const SizedBox(height: 12),
                    ..._xaiLines(item).map((line) => Padding(
                      padding: const EdgeInsets.only(bottom: 8),
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        const Padding(
                          padding: EdgeInsets.only(top: 2),
                          child: Icon(Icons.check_circle_rounded, color: green, size: 16),
                        ),
            ...
            Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [
              ...
              SizedBox(height: 10),
              Text('Arable — land that can support crop production under the assessed conditions.'),
              SizedBox(height: 6),
              Text('NDVI — satellite vegetation index from -1 to 1; higher positive values generally indicate denser vegetation.'),
              SizedBox(height: 6),
              Text('Suitability — the system’s comparative fit score, not a guarantee of yield.'),
              SizedBox(height: 6),
              Text('Slope — terrain steepness; the summary describes the dominant condition and localized steep sections.'),
              SizedBox(height: 6),
              Text('Wet/Dry season — seasonal context used to adjust, but not replace, the environmental ranking.'),
              SizedBox(height: 8),
              Text('Green: favorable  •  Yellow/Orange: conditional  •  Red: limiting/not suitable', style: TextStyle(fontWeight: FontWeight.w800, color: green)),
            ]))),
            const SizedBox(height: 10),
            Card(
              child: Padding(
            ...
            if (('${item['verification_status'] ?? 'draft'}' == 'draft') ||
              ...
              FilledButton.icon(
                onPressed: () async {
                  final isResubmission = '${item['verification_status'] ?? 'draft'}' == 'rejected';
                  final confirmed = await showDialog<bool>(
                    context: context,
                    builder: (dialogContext) => AlertDialog(
                      icon: const Icon(Icons.send_rounded, color: Color(0xFF0B7D49), size: 34),
                      title: Text(isResubmission ? 'Resubmit this analysis?' : 'Submit to planner?'),
                      content: Text(
                        isResubmission
                      ...
                      actions: [
                        ...
                        FilledButton.icon(
                          ...
                          label: Text(isResubmission ? 'Yes, Resubmit' : 'Yes, Submit'),
                        ),
                      ],
                    ),
                  );
                  ...
                  try {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Analysis submitted to the planner.')),
                    );
                ...
                icon: const Icon(Icons.send_rounded),
                label: Text('${item['verification_status'] ?? 'draft'}' == 'rejected'
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(_label, style: TextStyle(color: _color, fontWeight: FontWeight.w800, fontSize: 10)),
      ]),
    );
  }
}
...
Future<void> generateAnalysisPdf(Map<String, dynamic> raw, {AnalysisState? state}) async {
  ...
  final plantingMonthRaw = data['intended_planting_month'] ?? data['planting_month'];
  final plantingMonthIdx = plantingMonthRaw is num ? plantingMonthRaw.toInt() : int.tryParse('$plantingMonthRaw');
  final plantingMonth = (plantingMonthIdx != null && plantingMonthIdx >= 1 && plantingMonthIdx <= 12)
      ? monthNames[plantingMonthIdx] : null;

  dynamic rawXai = data['xai_explanation'];
  if (rawXai is String && rawXai.trim().isNotEmpty) {
    try { rawXai = jsonDecode(rawXai); } catch (_) {}
  }
  final Map xai = rawXai is Map ? rawXai : const {};
  final supportingFactors = (xai['supporting_factors'] is List) ? List.from(xai['supporting_factors']) : const [];
  final limitingFactors = (xai['limiting_factors'] is List) ? List.from(xai['limiting_factors']) : const [];

  final verificationStatus = (data['verification_status'] ?? 'draft').toString();
  ...
  if (rawQuality is String && rawQuality.trim().isNotEmpty) {
    try { rawQuality = jsonDecode(rawQuality); } catch (_) {}
  }
  final List<dynamic> qualityWarnings = rawQuality is List ? rawQuality : const [];
  ...
  doc.addPage(
    pw.MultiPage(
      build: (context) => [
        ...
        if (farmName != null) pw.Text('Farm: $farmName', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
        pw.Text('Location: $location'),
        pw.Text('Coordinates: $lat, $lon'),
        if (areaHectares != null)
          pw.Text('Approximate area: ${(areaHectares is num ? areaHectares.toStringAsFixed(3) : areaHectares)} hectares (agricultural estimate, not a cadastral survey)'),
        if (plantingMonth != null) pw.Text('Selected planting month: $plantingMonth'),
        ...
        pw.Text('Infrastructure Recommendation: ${data['infrastructure_recommendation'] ?? '--'}'),
        ...
        pw.Text('Phosphorus: ${data['phosphorus'] ?? '--'} (estimated from proxies, not a direct soil test)'),
        pw.Text('Potassium: ${data['potassium'] ?? '--'} (estimated from proxies, not a direct soil test)'),
        if (qualityWarnings.isNotEmpty) ...[
          ...
          pw.Container(
            ...
            child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
              pw.Text('Data quality notes:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
              for (final w in qualityWarnings) pw.Text('• $w', style: const pw.TextStyle(fontSize: 9)),
            ]),
          ),
        ],
        if (xai['summary'] != null) ...[
          pw.SizedBox(height: 14),
          pw.Text('Why this recommendation (explainability)', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 13)),
          pw.SizedBox(height: 4),
          pw.Text('${xai['summary']}', style: const pw.TextStyle(fontSize: 10)),
          if (supportingFactors.isNotEmpty) ...[
            pw.SizedBox(height: 6),
            pw.Text('Supporting factors:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
            for (final f in supportingFactors) pw.Text('• $f', style: const pw.TextStyle(fontSize: 9)),
          ],
          if (limitingFactors.isNotEmpty) ...[
            pw.SizedBox(height: 6),
            pw.Text('Limiting factors:', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
            for (final f in limitingFactors) pw.Text('• $f', style: const pw.TextStyle(fontSize: 9)),
          ],
          if ('${xai['comparison'] ?? ''}'.trim().isNotEmpty) ...[
            pw.SizedBox(height: 6),
            pw.Text('${xai['comparison']}', style: const pw.TextStyle(fontSize: 9, fontStyle: pw.FontStyle.italic)),
          ],
        ],
        pw.SizedBox(height: 14),
        pw.Text('Verification status', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 13)),
        pw.SizedBox(height: 4),
        pw.Text('Status: ${verificationStatus[0].toUpperCase()}${verificationStatus.substring(1)}'),
        if (verifiedAt != null) pw.Text('Reviewed: $verifiedAt'),
        if (plannerNotes != null && '$plannerNotes'.trim().isNotEmpty) pw.Text('Analyst notes: $plannerNotes'),
        ...
        pw.SizedBox(height: 8),
        pw.Text('Generated by GeoSustain mobile/web application.', style: const pw.TextStyle(fontSize: 8, color: PdfColors.grey700)),
      ],
    ),
  );
