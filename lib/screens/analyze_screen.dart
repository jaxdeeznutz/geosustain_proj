      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'This is the exact area that was analyzed.',
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Text(
                  '${state.numText(weather?['temperature_c'])} °C',
          ...
          children: [
            Row(
              children: [
                ...
                Text(
                  'pH ${state.numText(data?['soil_ph'])}${phEstimated ? ' (est.)' : ''}',
                ...
                Expanded(
                  child: Column(
                    children: [
                      ...
                      InfoProgress(
                        label: 'Phosphorus (estimated)',
                        value: data?['phosphorus'],
                      ),
                      InfoProgress(
                        label: 'Potassium (estimated)',
            ...
            const SizedBox(height: 10),
            Text(
              'NDVI: ${state.numText(data?['ndvi'])}${ndviEstimated ? ' (est.)' : ''}',
...
class ResultSummaryCard extends StatelessWidget {
  ...
  String friendlyLandStatus(Map<String, dynamic>? data) {
    ...
    if (raw.contains('WATER') || raw.contains('FLOOD')) {
      return 'This spot is water or flooded';
    }
    if (raw.contains('INFRASTRUCTURE') || raw.contains('BARE')) {
      return 'This looks like bare or built-up land';
    }
    if (raw.contains('DEGRADED') || raw.contains('LOW VEGETATION')) {
      return 'This soil could use some improvement';
    }
    if (raw.contains('ARABLE') || raw.contains('VEGETATED')) {
      return 'Good land for farming';
    }
    return data?['land_status'] != null
        ? 'Land condition analyzed'
        : 'No land analyzed yet.';
  }
  ...
  List<String> _recommendationReasons(Map<String, dynamic>? data) {
    ...
    if (xai != null) {
      ...
      if (supporting is List) {
        reasons.addAll(
          supporting.map((e) => '$e').where((e) => e.trim().isNotEmpty),
        );
      }
    ...
    return [
      'Run a new analysis to generate a parcel-specific Explainable AI result.',
    ];
  }
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              _SummaryBadge(
                text: isCrop
                    ? 'Match: ${_pctText(data?['crop_compatibility_pct'], 0)}%'
                ...
                children: [
                  Row(
                    children: const [
                      ...
                      SizedBox(width: 6),
                      Text(
                        'Some values below are estimates',
                        style: TextStyle(
                          color: Colors.white,
          ...
          const SizedBox(height: 8),
          Text(
            '${data?['predicted_crop'] ?? data?['crop_recommendation'] ?? data?['recommended_crop'] ?? '--'}'
                .toUpperCase(),
            style: const TextStyle(
              color: Colors.white,
              fontSize: 24,
            ...
            children: [
              Expanded(
                child: GreenMetric(
                  label: 'Environmental',
                  value: isCrop
                      ? '${state.numText(data?['environmental_suitability_pct'] ?? data?['crop_compatibility_pct'])}%'
                      : '--',
                ),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: GreenMetric(
                  ...
                  value: isCrop
                      ? '${state.numText(data?['season_adjusted_score'] ?? data?['crop_compatibility_pct'])}%'
                      : '--',
                ),
              ),
            ],
          ...
          if (isCrop) ...[
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'PLANTING CALENDAR — ${data?['intended_planting_month_name'] ?? state.intendedPlantingMonthName}'
                        .toUpperCase(),
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 11,
                  ...
                  const SizedBox(height: 4),
                  Text(
                    'Suggested window: ${data?['recommended_planting_window'] ?? '--'}',
                    style: TextStyle(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        isCrop ? 'WHY THIS CROP?' : 'WHY NO CROP YET?',
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 12,
                          fontWeight: FontWeight.w900,
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...alternatives.take(5).toList().asMap().entries.map((entry) {
                    ...
                    final crop = item is Map
                        ? (item['crop'] ?? item['name'] ?? '--')
                        : '$item';
                    final pct = item is Map
                        ? _pctText(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(
                color: infrastructureColor(d).withValues(alpha: 0.16),
                borderRadius: BorderRadius.circular(14),
                border: Border.all(
                  color: infrastructureColor(d).withValues(alpha: 0.45),
                ),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
                    '${d?['infrastructure_suitability'] ?? 'NOT YET ANALYZED'}',
                    style: TextStyle(
        ...
        child: Column(
          ...
          children: [
            ...
            GridView.count(
              ...
              children: geeItems(
                state,
              ).map((x) => MetricTile(item: x)).toList(),
            ),
          ],
        ),
      ),
              ...
              (x) => Padding(
                ...
                child: Row(
                  children: [
                    CircleAvatar(
                      radius: 16,
                      backgroundColor: x.color.withValues(alpha: 0.12),
