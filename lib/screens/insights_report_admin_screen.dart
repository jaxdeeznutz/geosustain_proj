              ...
              (b) => Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      '${b.$2.clamp(0, 100).toStringAsFixed(0)}%',
                      style: const TextStyle(
...
class ReportPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final monthNum = monthIdx is num
        ? monthIdx.toInt()
        : int.tryParse('$monthIdx');
    final plantingMonth = (monthNum != null && monthNum >= 1 && monthNum <= 12)
        ? monthNames[monthNum]
        : null;
    final xai = (d?['xai_explanation'] is Map)
        ? d!['xai_explanation'] as Map
        : const {};
    final supporting = (xai['supporting_factors'] is List)
        ? List.from(xai['supporting_factors'])
        : const [];
    final limiting = (xai['limiting_factors'] is List)
        ? List.from(xai['limiting_factors'])
        : const [];
    final verificationStatus = '${d?['verification_status'] ?? 'draft'}';
    final plannerNotes = d?['planner_notes'];
    final warnings = (d?['data_quality_warnings'] is List)
        ? List.from(d!['data_quality_warnings'])
        : const [];
    return Scaffold(
      backgroundColor: bg,
      body: SafeArea(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (areaHectares != null)
                      Text(
                        'Approximate area: ${(areaHectares is num ? areaHectares.toStringAsFixed(3) : areaHectares)} ha (agricultural estimate, not a legal survey)',
                      ),
                    if (plantingMonth != null)
                      Text('Selected planting month: $plantingMonth'),
                    ...
                    Text('Suitability Level: ${displaySuitability(d)}'),
                    const Divider(height: 28),
                    Text('NDVI: ${state.numText(d?['ndvi'])}'),
                    Text('Rainfall: ${state.numText(d?['rainfall_mm'])} mm'),
                    Text(
                      'Temperature: ${state.numText(d?['temperature_c'])} °C',
                    ),
                    Text('Elevation: ${state.numText(d?['elevation_m'])} m'),
                    Text('Soil pH: ${state.numText(d?['soil_ph'])}'),
                    const Divider(height: 28),
                    Text(
                      'Infrastructure Suitability: ${d?['infrastructure_suitability'] ?? '--'}',
                    ),
                    Text(
                      'Infrastructure Score: ${d?['infrastructure_score'] ?? '--'}',
                    ),
                    Text(
                      'Infrastructure Status: ${d?['infrastructure_status'] ?? '--'}',
                    ),
                    Text(
                      'Infrastructure Recommendation: ${d?['infrastructure_recommendation'] ?? '--'}',
                    ),
                    ...
                    if (xai['summary'] != null) ...[
                      ...
                      if (limiting.isNotEmpty) ...[
                        ...
                        for (final f in limiting)
                          Text('• $f', style: const TextStyle(fontSize: 12)),
                      ],
                    ],
                    const Divider(height: 28),
                    Text(
                      'Verification status: ${verificationStatus.isEmpty ? 'Draft' : '${verificationStatus[0].toUpperCase()}${verificationStatus.substring(1)}'}',
                      style: const TextStyle(fontWeight: FontWeight.w700),
                    ),
                    if (plannerNotes != null &&
                        '$plannerNotes'.trim().isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(top: 4),
                        child: Text('Analyst notes: $plannerNotes'),
                      ),
                    const SizedBox(height: 10),
                    const Text(
            ...
            children: [
              ...
              const Card(
                child: ListTile(
                  leading: CircleAvatar(
                    backgroundColor: softGreen,
                    child: Icon(Icons.cloud_done_outlined, color: green),
                  ),
                  title: Text('Live Weather Advisory'),
                  subtitle: Text(
                    'Open the Home page to refresh real-time weather alerts.',
                  ),
                ),
              ),
            ],
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              TextButton(
                                onPressed: st.weatherLoading
                                    ? null
                                    : () => st.refreshLiveWeather(),
                                child: Text(
                                  st.weatherLoading ? 'Updating...' : 'Refresh',
                                  style: const TextStyle(
                                    color: green,
                                    fontWeight: FontWeight.w800,
                                  ),
                  ...
                  const Card(
                    child: ListTile(
                      leading: CircleAvatar(
                        ...
                        child: Icon(Icons.info_outline, color: green),
                      ),
                      title: Text('Not connected to land analysis'),
                      subtitle: Text(
                        'These alerts are generated from live and upcoming weather data only. Crop analysis does not control this warning section.',
                      ),
                    ),
                  ),
                ],
