      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const Column(
            children: [
              Row(children: [
                LegendDot(color: Color(0xFF1FA463), text: '75-100% High'),
                SizedBox(width: 10),
                LegendDot(color: Color(0xFF8BDD75), text: '50-75% Moderate'),
              ]),
              SizedBox(height: 8),
              Row(children: [
                LegendDot(color: Color(0xFFE9A829), text: '25-50% Marginal'),
                SizedBox(width: 10),
                LegendDot(color: Color(0xFFE45B5B), text: '0-25% Low'),
              ]),
            ],
          ),
        ]),
...
class ReportPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final d = state.result;
    final farmName = d?['farm_name'];
    final areaHectares = d?['area_hectares'];
    const monthNames = ['', 'January', 'February', 'March', 'April', 'May', 'June',
        'July', 'August', 'September', 'October', 'November', 'December'];
    final monthIdx = d?['intended_planting_month'] ?? d?['planting_month'];
    final monthNum = monthIdx is num ? monthIdx.toInt() : int.tryParse('$monthIdx');
    final plantingMonth = (monthNum != null && monthNum >= 1 && monthNum <= 12) ? monthNames[monthNum] : null;
    final xai = (d?['xai_explanation'] is Map) ? d!['xai_explanation'] as Map : const {};
    final supporting = (xai['supporting_factors'] is List) ? List.from(xai['supporting_factors']) : const [];
    final limiting = (xai['limiting_factors'] is List) ? List.from(xai['limiting_factors']) : const [];
    final verificationStatus = '${d?['verification_status'] ?? 'draft'}';
    final plannerNotes = d?['planner_notes'];
    final warnings = (d?['data_quality_warnings'] is List) ? List.from(d!['data_quality_warnings']) : const [];
    return Scaffold(
      backgroundColor: bg,
      body: SafeArea(
                      ...
                      children: [
                        const Text('GeoSustain Analysis Report',
                            style: TextStyle(
                                ...
                                color: green)),
                        const SizedBox(height: 10),
                        if (farmName != null) Text('Farm: $farmName', style: const TextStyle(fontWeight: FontWeight.w800)),
                        Text(
                            'Location: ${d?['place_name'] ?? state.selectedPlaceName}'),
                        Text(
                            'Coordinates: ${state.latController.text}, ${state.lonController.text}'),
                        if (areaHectares != null)
                          Text('Approximate area: ${(areaHectares is num ? areaHectares.toStringAsFixed(3) : areaHectares)} ha (agricultural estimate, not a legal survey)'),
                        if (plantingMonth != null) Text('Selected planting month: $plantingMonth'),
                        ...
                        if (warnings.isNotEmpty) ...[
                          const Divider(height: 28),
                          const Text('Data quality notes', style: TextStyle(fontWeight: FontWeight.w800)),
                          const SizedBox(height: 4),
                          for (final w in warnings) Padding(padding: const EdgeInsets.only(bottom: 2), child: Text('• $w', style: const TextStyle(fontSize: 12, color: Colors.black54))),
                        ],
                        if (xai['summary'] != null) ...[
                          const Divider(height: 28),
                          const Text('Why this recommendation', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
                          const SizedBox(height: 6),
                          Text('${xai['summary']}', style: const TextStyle(fontSize: 13)),
                          if (supporting.isNotEmpty) ...[
                            const SizedBox(height: 8),
                            const Text('Supporting factors', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                            for (final f in supporting) Text('• $f', style: const TextStyle(fontSize: 12)),
                          ],
                          if (limiting.isNotEmpty) ...[
                            const SizedBox(height: 8),
                            const Text('Limiting factors', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12)),
                            for (final f in limiting) Text('• $f', style: const TextStyle(fontSize: 12)),
                          ],
                        ],
                        const Divider(height: 28),
                        Text('Verification status: ${verificationStatus.isEmpty ? 'Draft' : '${verificationStatus[0].toUpperCase()}${verificationStatus.substring(1)}'}', style: const TextStyle(fontWeight: FontWeight.w700)),
                        if (plannerNotes != null && '$plannerNotes'.trim().isNotEmpty)
                          Padding(padding: const EdgeInsets.only(top: 4), child: Text('Analyst notes: $plannerNotes')),
                        const SizedBox(height: 10),
                        const Text(
                          'This report is an AI-assisted agricultural estimate, not a certified soil test or legal land survey.',
