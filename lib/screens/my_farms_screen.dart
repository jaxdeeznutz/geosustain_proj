...
class _MyFarmsPageState extends State<MyFarmsPage> {
  ...
  double _areaHectares(List<LatLng> pts) {
    ...
    const r = 6378137.0;
    final lat0 = (pts.map((p) => p.latitude).reduce((a, b) => a + b) / pts.length) * math.pi / 180;
    final xy = pts
        .map((p) => Offset(
  ...
  double _closureGapMeters(List<LatLng> pts) {
    ...
    return Geolocator.distanceBetween(
        pts.first.latitude, pts.first.longitude, pts.last.latitude, pts.last.longitude);
  }
  ...
  List<LatLng> _simplifyPath(List<LatLng> points, {double toleranceMeters = 2.5}) {
    if (points.length < 3) return points;
    const r = 6378137.0;
    final lat0 = (points.map((p) => p.latitude).reduce((a, b) => a + b) / points.length) * math.pi / 180;
    final xy = points
        .map((p) => Offset(
              r * (p.longitude * math.pi / 180) * math.cos(lat0),
              r * (p.latitude * math.pi / 180),
    ...
    void recurse(int start, int end) {
      ...
      for (var i = start + 1; i < end; i++) {
        ...
        if (abLen == 0) {
          ...
          final t = (((p.dx - a.dx) * (b.dx - a.dx)) + ((p.dy - a.dy) * (b.dy - a.dy))) / (abLen * abLen);
          final tt = t.clamp(0.0, 1.0);
          final proj = Offset(a.dx + tt * (b.dx - a.dx), a.dy + tt * (b.dy - a.dy));
  ...
  Future<void> _testLocation() async {
    ...
    try {
      ...
      _snack('Got a fix: ${pos.latitude.toStringAsFixed(5)}, ${pos.longitude.toStringAsFixed(5)} (±${pos.accuracy.toStringAsFixed(0)}m)');
    } catch (e) {
  ...
  Future<void> _startWalk() async {
    ...
    if (permission == LocationPermission.denied ||
      ...
      _snack('Location permission is required to walk your farm boundary. Please allow it in your device settings.');
      return;
    }
    ...
    _tickTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted) setState(() {}); // keeps the "last update Xs ago" readout live
    });
    ...
    if (!kIsWeb && defaultTargetPlatform == TargetPlatform.android) {
      ...
      walkLocationSettings = const LocationSettings(accuracy: LocationAccuracy.best, distanceFilter: 3);
    }
    walkSub = Geolocator.getPositionStream(
      ...
      if (p.accuracy > 20) {
        setState(() => liveStatusNote = 'Waiting for a more accurate GPS fix (±${p.accuracy.toStringAsFixed(0)}m) — point not recorded yet.');
        return;
      }
      if (walkPoints.isNotEmpty) {
        final prev = walkPoints.last;
        final jump = Geolocator.distanceBetween(prev.latitude, prev.longitude, next.latitude, next.longitude);
        ...
        if (jump > 80) {
          setState(() => liveStatusNote = 'Ignored a GPS jump of ${jump.toStringAsFixed(0)}m — that looked like a bad reading, not a real step.');
          return;
        }
        distanceM += jump;
      }
  ...
  Future<void> _finishWalk() async {
    ...
    if (walkPoints.length < 3) {
      _snack('Walk farther around the parcel. At least three valid GPS points are required.');
      return;
    }
    ...
    final finalPoints = simplified.length >= 3 ? simplified : List<LatLng>.from(walkPoints);
    setState(() {
      walking = false;
  ...
  void _saveManual() {
    ...
    if (points.length < 3) {
      _snack('Draw a polygon on the Map page first, then return here to save it.');
      return;
    }
    setState(() {
      reviewPoints = points;
  ...
  Future<void> _saveFarm(List<LatLng> points, String method, double? accuracy) async {
    final name = TextEditingController();
    final location = TextEditingController(text: widget.state.selectedPlaceName);
    final ok = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Save Farm Parcel'),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: name, decoration: const InputDecoration(labelText: 'Farm name', hintText: 'e.g., Riverside Rice Field')),
          const SizedBox(height: 12),
          TextField(controller: location, decoration: const InputDecoration(labelText: 'Location description')),
          const SizedBox(height: 12),
          Text('Estimated area: ${_areaHectares(points).toStringAsFixed(3)} ha', style: const TextStyle(fontWeight: FontWeight.w700)),
          const SizedBox(height: 6),
          const Text('This is an agricultural estimate, not a legal cadastral survey.', style: TextStyle(fontSize: 12, color: Colors.black54)),
        ]),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Save Farm')),
        ],
      ),
    );
    ...
    try {
      ...
      _snack('Farm parcel saved. You can reuse this boundary for future wet- or dry-season analyses.');
      _discardReview();
      await _load();
    } catch (e) {
  ...
  Future<void> _analyze(Map<String, dynamic> farm) async {
    ...
    if (err != null) _snack(err); else if (mounted) Navigator.pop(context, true);
  }
  ...
  Widget _buildWalkingMap() {
    final center = liveRaw ?? (walkPoints.isNotEmpty ? walkPoints.last : const LatLng(7.30, 125.65));
    return ClipRRect(
      ...
      child: SizedBox(
        ...
        child: FlutterMap(
          ...
          options: MapOptions(initialCenter: center, initialZoom: 17, interactionOptions: const InteractionOptions(flags: InteractiveFlag.all & ~InteractiveFlag.rotate)),
          children: [
            ...
            if (walkPoints.length >= 2)
              PolylineLayer(polylines: [Polyline(points: walkPoints, color: green, strokeWidth: 4)]),
            if (walkPoints.length >= 3)
              PolygonLayer(polygons: [Polygon(points: walkPoints, color: green.withOpacity(0.18), borderColor: green, borderStrokeWidth: 2)]),
            MarkerLayer(markers: [
              ...walkPoints.asMap().entries.map((e) => Marker(
                    point: e.value,
                    width: 10,
                    height: 10,
                    child: Container(decoration: BoxDecoration(color: green, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 1.5))),
                  )),
              if (liveRaw != null)
                Marker(
                  point: liveRaw!,
  ...
  Widget _buildWalkingCard() {
    final approxArea = walkPoints.length >= 3 ? _areaHectares(walkPoints) : null;
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: const [
            ...
            SizedBox(width: 8),
            Text('Walking Boundary', style: TextStyle(fontWeight: FontWeight.w900, color: green, fontSize: 16)),
          ]),
          ...
          Wrap(spacing: 8, runSpacing: 8, children: [
            ...
            Chip(label: Text(lastAccuracy == null ? 'GPS accuracy: --' : 'GPS accuracy: ±${lastAccuracy!.toStringAsFixed(1)} m')),
            if (approxArea != null) Chip(label: Text('~${approxArea.toStringAsFixed(3)} ha so far')),
          ]),
          const SizedBox(height: 6),
          Row(children: [
            Icon(rawFixCount > 0 ? Icons.sensors_rounded : Icons.sensors_off_rounded,
                size: 14, color: rawFixCount > 0 ? green : Colors.redAccent),
            ...
            Expanded(
              child: Text(
                ...
                style: TextStyle(
                  fontSize: 11,
                  color: rawFixCount == 0 ? Colors.redAccent : Colors.black45,
                  fontWeight: rawFixCount == 0 ? FontWeight.w700 : FontWeight.w400,
                ),
              ),
            ),
          ]),
          if (liveStatusNote != null) ...[
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Icon(Icons.gps_not_fixed_rounded, size: 16, color: Colors.black45),
              const SizedBox(width: 6),
              Expanded(child: Text(liveStatusNote!, style: const TextStyle(fontSize: 12, color: Colors.black54))),
            ]),
          ],
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: OutlinedButton.icon(onPressed: _cancelWalk, icon: const Icon(Icons.close_rounded), label: const Text('Cancel'))),
            const SizedBox(width: 10),
            Expanded(child: FilledButton.icon(onPressed: _finishWalk, icon: const Icon(Icons.stop_circle_outlined), label: const Text('Finish Walking'))),
          ]),
        ]),
      ),
    );
  ...
  Widget _buildReviewCard() {
    ...
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(14),
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          const Text('Review Boundary', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16)),
          const SizedBox(height: 6),
          Text(reviewMethod == 'gps_walk' ? 'Recorded by walking the parcel with GPS.' : 'Drawn manually on the map.', style: const TextStyle(color: Colors.black54)),
          const SizedBox(height: 10),
          ClipRRect(
            borderRadius: BorderRadius.circular(12),
            child: SizedBox(
              ...
              child: FlutterMap(
                options: MapOptions(
                  initialCameraFit: CameraFit.coordinates(coordinates: points, padding: const EdgeInsets.all(32)),
                ...
                children: [
                  ...
                  MarkerLayer(markers: points.asMap().entries.map((e) => Marker(
                        point: e.value,
                        width: 22,
                        height: 22,
                        child: CircleAvatar(backgroundColor: green, radius: 9, child: Text('${e.key + 1}', style: const TextStyle(fontSize: 9, color: Colors.white, fontWeight: FontWeight.w800))),
                      )).toList()),
                ],
              ),
            ),
          ...
          const SizedBox(height: 10),
          Wrap(spacing: 8, runSpacing: 8, children: [
            Chip(label: Text(reviewRawPointCount != null && reviewRawPointCount! > points.length
                ? '${points.length} vertices (simplified from $reviewRawPointCount walked points)'
                : '${points.length} vertices')),
            Chip(label: Text('Estimated area: ${_areaHectares(points).toStringAsFixed(3)} ha')),
          ]),
                ...
                for (final w in warnings)
                  Padding(
                    ...
                    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Icon(Icons.warning_amber_rounded, size: 18, color: Color(0xFFB26A00)),
                      const SizedBox(width: 6),
                      Expanded(child: Text(w, style: const TextStyle(fontSize: 12.5, color: Color(0xFF7A4A00)))),
                    ]),
                  ),
              ]),
            ),
          ...
          const SizedBox(height: 6),
          const Text('This boundary is an agricultural estimate, not a legal cadastral survey.', style: TextStyle(fontSize: 12, color: Colors.black54)),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: OutlinedButton.icon(onPressed: _discardReview, icon: const Icon(Icons.undo_rounded), label: const Text('Discard & Redraw'))),
            const SizedBox(width: 10),
            Expanded(child: FilledButton.icon(onPressed: _confirmReview, icon: const Icon(Icons.check_circle_outline_rounded), label: const Text('Looks Good — Save'))),
          ]),
        ]),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('My Farms'), backgroundColor: Colors.white),
      body: ListView(padding: const EdgeInsets.all(14), children: [
        if (reviewPoints != null)
          ...
          Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            const Text('Register a Farm Parcel', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
            const SizedBox(height: 6),
            const Text('Walk the boundary using GPS or save the polygon drawn on the map. Each farm is stored separately and can be analyzed many times.', style: TextStyle(color: Colors.black54, height: 1.35)),
            const SizedBox(height: 14),
            FilledButton.icon(onPressed: _startWalk, icon: const Icon(Icons.directions_walk_rounded), label: const Text('Walk My Farm Boundary')),
            const SizedBox(height: 8),
            OutlinedButton.icon(onPressed: _saveManual, icon: const Icon(Icons.draw_rounded), label: const Text('Save Boundary Drawn on Map')),
            const SizedBox(height: 8),
            TextButton.icon(onPressed: _testLocation, icon: const Icon(Icons.gps_fixed_rounded, size: 16), label: const Text('Test my GPS location', style: TextStyle(fontSize: 12))),
          ]))),
        const SizedBox(height: 14),
        Row(children: [const Expanded(child: Text('Saved Farm Parcels', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900))), IconButton(onPressed: _load, icon: const Icon(Icons.refresh))]),
        if (loading)
          const Center(child: Padding(padding: EdgeInsets.all(32), child: CircularProgressIndicator()))
        else if (farms.isEmpty)
          const Card(child: Padding(padding: EdgeInsets.all(24), child: Text('No farms saved yet. Walk or draw your first parcel boundary above.', textAlign: TextAlign.center)))
        else
          ...farms.map((farm) => Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  const Icon(Icons.landscape_rounded, color: green),
                  const SizedBox(width: 10),
                  Expanded(child: Text('${farm['farm_name']}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900))),
                  PopupMenuButton<String>(onSelected: (v) async {
                    if (v == 'archive') {
                      await widget.state.api.updateFarm(int.parse('${farm['id']}'), {'is_archived': true});
                      _load();
                    }
                  }, itemBuilder: (_) => const [PopupMenuItem(value: 'archive', child: Text('Archive farm'))]),
                ]),
                const SizedBox(height: 6),
                Text('${farm['location_name'] ?? 'Location not named'}', style: const TextStyle(color: Colors.black54)),
                const SizedBox(height: 8),
                Wrap(spacing: 8, runSpacing: 8, children: [
                  Chip(label: Text('${(double.tryParse('${farm['area_hectares']}') ?? 0).toStringAsFixed(3)} ha')),
                  Chip(label: Text('${farm['mapping_method'] == 'gps_walk' ? 'GPS walked' : 'Map drawn'}')),
                  Chip(label: Text('${farm['analysis_count'] ?? 0} analyses')),
                ]),
                const SizedBox(height: 8),
                SizedBox(width: double.infinity, child: FilledButton.icon(onPressed: () => _analyze(farm), icon: const Icon(Icons.analytics_outlined), label: const Text('Analyze This Farm'))),
              ]))))
              .toList(),
      ]),
    );
