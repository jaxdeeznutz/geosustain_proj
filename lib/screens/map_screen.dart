...
class _LiveFieldMapState extends State<LiveFieldMap> {
  ...
  double? _num(dynamic value) => value is num ? value.toDouble() : double.tryParse('$value');

  String _heatmapKey(String layer) => const {
    'NDVI': 'ndvi',
  ...
  Color? _polygonHeatColor(AnalysisState s) {
    ...
    final t = maxV == minV ? .75 : ((average - minV) / (maxV - minV)).clamp(0.0, 1.0);
  ...
  Color _heatColor(double value, double minV, double maxV) {
    final t = maxV == minV ? .65 : ((value - minV) / (maxV - minV)).clamp(0.0, 1.0);
  ...
  List<Polygon> _heatCells(AnalysisState s, List<LatLng> boundary) {
    ...
    final lats=boundary.map((p)=>p.latitude).toList(), lngs=boundary.map((p)=>p.longitude).toList();
    final latStep=((lats.reduce(math.max)-lats.reduce(math.min))/7).clamp(.00008,.0012).toDouble();
    final lngStep=((lngs.reduce(math.max)-lngs.reduce(math.min))/7).clamp(.00008,.0012).toDouble();
    final cells=<Polygon>[];
    for(final item in grid){
      final lat=_num(item['lat']), lng=_num(item['lng']), value=_num(item[key]);
      if(lat==null||lng==null||value==null) continue;
      cells.add(Polygon(points:[
        LatLng(lat-latStep,lng-lngStep), LatLng(lat-latStep,lng+lngStep),
        LatLng(lat+latStep,lng+lngStep), LatLng(lat+latStep,lng-lngStep),
      ], color:_heatColor(value,minV,maxV).withOpacity(.62), borderColor:Colors.white.withOpacity(.45), borderStrokeWidth:1));
    }
    return cells;
  }
        ...
        if (points.length >= 3)
          PolygonLayer(
            polygons: [
              Polygon(
                points: points,
                color: _heatCells(s, points).isNotEmpty ? Colors.transparent : (_polygonHeatColor(s) ?? green).withOpacity(_polygonHeatColor(s) == null ? 0.18 : 0.46),
          ...
          child: SingleChildScrollView(
            ...
            child: Row(children: [
              ...
              if (state.result?['heatmap_grid'] is List &&
                ...
                ...['NDVI', 'Suitability', 'Soil pH', 'Rainfall', 'Elevation', 'Slope']
                    .map((layer) => MiniPill(
                          text: layer,
    ...
    return SafeArea(
      child: ListView(padding: const EdgeInsets.fromLTRB(14, 0, 14, 18), children: [
        ...
        if (state.result?['heatmap_grid'] is List &&
          ...
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
            decoration: BoxDecoration(color: const Color(0xFFFFF7E2), borderRadius: BorderRadius.circular(12)),
            child: const Row(children: [
              Icon(Icons.info_outline_rounded, color: Color(0xFFB47800), size: 19),
              SizedBox(width: 8),
              Expanded(child: Text(
                'Heatmaps appear after you finish and analyze a polygon boundary. They do not appear while you are still drawing.',
                style: TextStyle(fontSize: 12, color: Color(0xFF7A5600), fontWeight: FontWeight.w600),
              )),
            ]),
          ),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              DropdownButtonFormField<int>(
                ...
                decoration: const InputDecoration(
                  ...
                  helperText: 'The crop ranking will be adjusted using the local planting calendar.',
                ),
              ...
              SizedBox(width: double.infinity, child: OutlinedButton.icon(
                onPressed: () async { await Navigator.push(context, MaterialPageRoute(builder: (_) => MyFarmsPage(state: state))); },
              ...
              Text('Tap the map to draw the formal farm parcel boundary. Single-point analysis is not used for farmer submissions. Points: ${state.polygonPoints.length}', style: const TextStyle(color: Colors.black54, fontSize: 12)),
              const SizedBox(height: 12),
              Row(children: [
  ...
  Widget build(BuildContext context) {
    Widget item(Color color, String title, String meaning) => Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(width: 8),
        Expanded(child: RichText(text: TextSpan(style: const TextStyle(fontSize: 11.5, color: Colors.black87, height: 1.3), children: [
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [Icon(Icons.info_outline_rounded, size: 17, color: green), SizedBox(width: 7), Text('SEASON STATUS LEGEND', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: green))]),
        const SizedBox(height: 9),
        item(const Color(0xFF168A4A), 'In season', 'month is inside the preferred local planting window.'),
        item(const Color(0xFF57A96B), 'Preferred establishment', 'best month for establishing a perennial crop.'),
        item(const Color(0xFFE6A21A), 'Year-round with management', 'crop may be planted, but irrigation or moisture management is important.'),
        item(const Color(0xFFD9534F), 'Outside preferred season', 'score is reduced; consider the suggested planting window.'),
      ]),
    );
  }
}
