...
double? _recordAreaHectares(Map<String, dynamic> record) {
  ...
  if (polygon is String && polygon.trim().isNotEmpty) {
    try { polygon = jsonDecode(polygon); } catch (_) {}
  }
  if (polygon is Map) {
  ...
  for (final item in polygon) {
    try {
      if (item is Map) {
        ...
        final a = item[0] is num ? (item[0] as num).toDouble() : double.tryParse('${item[0]}');
        final b = item[1] is num ? (item[1] as num).toDouble() : double.tryParse('${item[1]}');
  ...
  final meanLat = points.map((p) => p.latitude).reduce((a, b) => a + b) / points.length * math.pi / 180;
  final xy = points.map((p) => Offset(
...
class _WebMapScreenState extends State<WebMapScreen> {
  ...
  List<Map<String, dynamic>> get _allMapRecords {
    ...
    for (final raw in [...state.historyRecords, ...state.plannerQueue]) {
      ...
      final fallback = '${record['center_lat']}:${record['center_lon']}:${record['analyzed_at']}';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) => _loadFarmerSubmissionsForMap());
  }

  LatLng? _pointFromRecord(Map<String, dynamic> record) {
    final latRaw = record['center_lat'] ?? record['lat'] ?? record['latitude'];
    final lonRaw = record['center_lon'] ?? record['lon'] ?? record['lng'] ?? record['longitude'];
    final lat = latRaw is num ? latRaw.toDouble() : double.tryParse('$latRaw');
    final lon = lonRaw is num ? lonRaw.toDouble() : double.tryParse('$lonRaw');
  ...
  List<LatLng> _polygonFromRecord(Map<String, dynamic> record) {
    ...
    if (raw is Map) {
      ...
      if ('${raw['type'] ?? ''}'.toLowerCase() == 'polygon' && coordinates is List && coordinates.isNotEmpty) {
  ...
  double? _suitability(Map<String, dynamic> record) {
    final raw = record['crop_compatibility_pct'] ?? record['compatibility_pct'] ?? record['suitability'];
  ...
  void _select(Map<String, dynamic> record) {
    ...
    setState(() => _selected = record.isEmpty ? null : Map<String, dynamic>.from(record));
  }

  Future<void> _openDetails(BuildContext context, Map<String, dynamic> record) async {
  ...
  Widget build(BuildContext context) {
    ...
    final validSuitability = mapRecords.map(_suitability).whereType<double>().toList();
      ...
      children: [
          ...
          child: _WebCard(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Padding(
                ...
                child: Row(children: [
                  const Expanded(
                    child: Text('Interactive Review Map', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
                  ),
                  _ToolbarChip(icon: Icons.layers_rounded, label: state.satellite ? 'Satellite' : 'Street', onTap: state.toggleSatellite),
                  const SizedBox(width: 8),
                  _ToolbarChip(icon: Icons.filter_alt_outlined, label: 'All Markers  ${mapRecords.length}', onTap: () {}),
                ]),
              ),
        ...
        SizedBox(
          ...
          child: Column(children: [
            _WebCard(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('GIS Review Summary', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
                const SizedBox(height: 14),
                _SummaryMetric(icon: Icons.location_on_rounded, label: 'Total Analyzed Lands', value: '${mapRecords.length}', caption: 'Across all saved analyses'),
                const Divider(height: 24),
                _SummaryMetric(icon: Icons.eco_rounded, label: 'Average Suitability', value: average == null ? '--' : '${average.toStringAsFixed(1)}%', caption: 'Across all analyzed lands'),
                const Divider(height: 24),
                _SummaryMetric(icon: Icons.trending_up_rounded, label: 'Infrastructure Overlays', value: 'Ready', caption: 'Available during land review'),
              ]),
            ),
            ...
            _WebCard(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  const Expanded(child: Text('Recent Analyzed Lands', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900))),
                  Text('${rows.length} shown', style: const TextStyle(color: Colors.black45, fontSize: 12)),
                ]),
                ...
                if (rows.isEmpty)
                  ...
                  ...rows.map((r) => _RecentLandRow(
                    record: r,
                    selected: identical(r, _selected) || (r['session_id'] != null && r['session_id'] == _selected?['session_id']),
                    place: _place(r),
                    crop: _crop(r),
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(foregroundColor: green, padding: const EdgeInsets.symmetric(vertical: 15)),
                  ),
                ),
              ]),
            ),
...
class _SummaryMetric extends StatelessWidget {
  ...
  const _SummaryMetric({required this.icon, required this.label, required this.value, required this.caption});

  @override
  Widget build(BuildContext context) => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Container(width: 46, height: 46, decoration: BoxDecoration(color: softGreen, borderRadius: BorderRadius.circular(14)), child: Icon(icon, color: green)),
    const SizedBox(width: 12),
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(color: Colors.black54, fontSize: 12, fontWeight: FontWeight.w700)),
      const SizedBox(height: 2),
      Text(value, style: const TextStyle(color: green, fontSize: 21, fontWeight: FontWeight.w900)),
      Text(caption, style: const TextStyle(color: Colors.black45, fontSize: 11)),
    ])),
  ]);
}
...
class _RecentLandRow extends StatelessWidget {
  ...
  final Color color;
  final VoidCallback onTap;
  const _RecentLandRow({required this.record, required this.selected, required this.place, required this.crop, required this.suitability, required this.color, required this.onTap});

  @override
  Widget build(BuildContext context) {
    final date = '${record['date'] ?? record['analyzed_at'] ?? ''}'.split('T').first;
    return InkWell(
      onTap: onTap,
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: selected ? green.withOpacity(.28) : Colors.transparent),
        ),
        child: Row(children: [
          Container(width: 39, height: 39, decoration: BoxDecoration(color: color.withOpacity(.13), shape: BoxShape.circle), child: Icon(Icons.eco_rounded, color: color, size: 21)),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(place, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
            const SizedBox(height: 2),
            Text('$crop${date.isEmpty ? '' : ' • $date'}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.black45, fontSize: 11)),
          ])),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6),
            decoration: BoxDecoration(color: color.withOpacity(.12), borderRadius: BorderRadius.circular(20)),
            child: Text(suitability == null ? '--' : '${suitability!.toStringAsFixed(1)}%', style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 12)),
          ),
        ]),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    for (final record in records.take(60)) {
      ...
      if (boundary.length >= 3) {
        parcelPolygons.add(Polygon(
          points: boundary,
          color: color.withOpacity(isSelected ? .28 : .10),
          borderColor: isSelected ? color : color.withOpacity(.65),
        ...
        child: Stack(children: [
          FlutterMap(
            ...
            children: [
              ...
              if (parcelPolygons.isNotEmpty) PolygonLayer(polygons: parcelPolygons),
          ...
          Positioned(
            ...
            bottom: 18,
            child: Column(children: [
              _MapControl(icon: Icons.add_rounded, onTap: () { try { mapController.move(mapController.camera.center, mapController.camera.zoom + 1); } catch (_) {} }),
              const SizedBox(height: 6),
              _MapControl(icon: Icons.remove_rounded, onTap: () { try { mapController.move(mapController.camera.center, mapController.camera.zoom - 1); } catch (_) {} }),
              const SizedBox(height: 6),
              _MapControl(icon: Icons.my_location_rounded, onTap: () { try { mapController.move(selectedPoint ?? state.selectedPoint, 13.2); } catch (_) {} }),
            ]),
          ),
        ]),
      ),
...
class _MapControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Material(
    ...
    elevation: 2,
    child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(12), child: SizedBox(width: 43, height: 43, child: Icon(icon, color: const Color(0xFF26352C)))),
  );
}
...
class _MarkerInfoCard extends StatelessWidget {
  ...
  const _MarkerInfoCard({required this.record, required this.place, required this.crop, required this.suitability, required this.suitabilityLabel, required this.markerColor, required this.onClose, required this.onOpenDetails});

  @override
  Widget build(BuildContext context) {
    final date = '${record['date'] ?? record['analyzed_at'] ?? ''}'.split('T').first;
    final areaHa = _recordAreaHectares(record);
    final area = areaHa == null ? '--' : '${areaHa.toStringAsFixed(areaHa < 1 ? 3 : 2)} ha';
    ...
    final scoreValue = isCrop && suitability != null ? '${suitability!.toStringAsFixed(1)}%' : 'Not recommended';
    ...
    return Material(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xFFE0E9E2))),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(width: 38, height: 38, decoration: BoxDecoration(color: markerColor.withOpacity(.13), shape: BoxShape.circle), child: Icon(Icons.eco_rounded, color: markerColor, size: 21)),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(place, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w900)),
              const SizedBox(height: 2),
              Text('$primaryLabel: $crop', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.black54, fontSize: 11)),
            ])),
            InkWell(onTap: onClose, child: const Padding(padding: EdgeInsets.all(3), child: Icon(Icons.close_rounded, size: 18))),
          ]),
          const Divider(height: 16),
          Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(scoreLabel, style: const TextStyle(color: Colors.black45, fontSize: 11)),
              Text(scoreValue, style: TextStyle(color: markerColor, fontSize: isCrop ? 24 : 17, fontWeight: FontWeight.w900)),
            ])),
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7), decoration: BoxDecoration(color: markerColor.withOpacity(.12), borderRadius: BorderRadius.circular(20)), child: Text(badgeText, style: TextStyle(color: markerColor, fontWeight: FontWeight.w900, fontSize: 11))),
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: _MiniInfo(icon: Icons.calendar_month_rounded, label: 'Date', value: date.isEmpty ? '--' : date)),
            Expanded(child: _MiniInfo(icon: Icons.crop_square_rounded, label: 'Area', value: area)),
          ]),
          const SizedBox(height: 12),
          SizedBox(width: double.infinity, child: OutlinedButton(onPressed: onOpenDetails, style: OutlinedButton.styleFrom(foregroundColor: green, backgroundColor: softGreen, side: BorderSide.none), child: const Text('View Details'))),
        ]),
      ),
    );
  }
