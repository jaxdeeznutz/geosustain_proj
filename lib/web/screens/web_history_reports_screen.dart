  ...
  Widget build(BuildContext context) {
    ...
    return _WebCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (rows.isEmpty) const Padding(padding: EdgeInsets.all(28), child: Center(child: Text('No analysis history yet.', style: TextStyle(color: Colors.black45))))
        ...
        final pctRaw = r['crop_compatibility_pct'] ?? r['compatibility_pct'];
        final pct = pctRaw is num ? pctRaw.toDouble() : double.tryParse('$pctRaw');
        return InkWell(
          ...
          onTap: () async {
            ...
            await showDialog<void>(
              ...
              builder: (_) => _WebHistoryDetailDialog(
                record: state.normalizeRecord(Map<String, dynamic>.from(r)),
                state: state,
          ...
          child: Container(
            ...
            child: Row(children: [
              Container(width: 44, height: 44, decoration: BoxDecoration(color: softGreen, borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.eco_rounded, color: green)),
              const SizedBox(width: 12),
              Expanded(flex: 22, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('${r['place_name'] ?? r['location'] ?? 'Analyzed Area'}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w900)),
                const SizedBox(height: 4),
                Text('${r['date'] ?? r['analyzed_at'] ?? ''}'.split('T').first, style: const TextStyle(color: Colors.black45, fontSize: 12)),
              ])),
              Expanded(flex: 14, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('RECOMMENDED CROP', style: TextStyle(fontSize: 10, color: Colors.black45, fontWeight: FontWeight.w800)),
                const SizedBox(height: 4),
                Text(_topCropFromRecord(r), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900)),
              ])),
              SizedBox(width: 110, child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                const Text('SUITABILITY', style: TextStyle(fontSize: 10, color: Colors.black45, fontWeight: FontWeight.w800)),
                const SizedBox(height: 4),
                Text(pct == null ? '--' : '${pct.toStringAsFixed(1)}%', style: const TextStyle(color: green, fontSize: 16, fontWeight: FontWeight.w900)),
              ])),
...
class WebReportsScreen extends StatelessWidget {
  ...
  Future<void> _downloadReport(BuildContext context, Map<String, dynamic> raw) async {
  ...
  Widget build(BuildContext context) {
    ...
    return _WebCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (rows.isEmpty) const Padding(padding: EdgeInsets.all(28), child: Center(child: Text('No reports generated yet.', style: TextStyle(color: Colors.black45))))
        ...
        final title = '${r['report_title'] ?? r['title'] ?? 'GeoSustain Analysis Report'}';
        final date = '${r['report_created_at'] ?? r['created_at'] ?? r['date'] ?? 'Recently generated'}';
        return ListTile(
          leading: const CircleAvatar(backgroundColor: Color(0xFFFFEBEB), child: Icon(Icons.picture_as_pdf_rounded, color: Colors.red)),
          title: Text(title, style: const TextStyle(fontWeight: FontWeight.w800)),
          subtitle: Text('${item['predicted_crop'] ?? 'Land analysis'} · $date'),
          trailing: IconButton(
...
class _WebHistoryDetailDialog extends StatefulWidget {
  ...
  @override
  State<_WebHistoryDetailDialog> createState() => _WebHistoryDetailDialogState();
}
...
class _WebHistoryDetailDialogState extends State<_WebHistoryDetailDialog> {
  ...
  Future<void> _addToReports() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(added ? 'Added to Reports. You can download it from the Reports page.' : 'This analysis is already in Reports.')),
      );
    } catch (e) {
      if (!mounted) return;
            ...
            Flexible(
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    Text('$place', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: green)),
                    const SizedBox(height: 6),
                    Text(state.recommendationText(record), style: const TextStyle(color: Colors.black54, height: 1.35)),
                    const SizedBox(height: 8),
                    Text(topCrop, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
                    ...
                    const SizedBox(height: 8),
                    Text('Coordinates: $lat, $lon', style: const TextStyle(fontSize: 13, color: Colors.black54)),
                    if (record['date'] != null) ...[
                      const SizedBox(height: 4),
                      Text('Analyzed: ${record['date']}', style: const TextStyle(fontSize: 13, color: Colors.black54)),
                    ],
                    const Divider(height: 28),
                    const Text('ENVIRONMENTAL DATA', style: TextStyle(fontWeight: FontWeight.w900, color: green, letterSpacing: 0.4)),
                    const SizedBox(height: 10),
                    DetailLine(label: 'NDVI', value: _num(record['ndvi'], decimals: 3)),
                    DetailLine(label: 'Rainfall', value: '${_num(record['rainfall_mm'], decimals: 1)} mm'),
                    DetailLine(label: 'Temperature', value: '${_num(record['temperature_c'], decimals: 1)} °C'),
                    DetailLine(label: 'Elevation', value: '${_num(record['elevation_m'], decimals: 1)} m'),
                    DetailLine(label: 'Soil pH', value: _num(record['soil_ph'], decimals: 2)),
                    DetailLine(label: 'Nitrogen', value: _num(record['nitrogen'], decimals: 0)),
                    DetailLine(label: 'Phosphorus', value: _num(record['phosphorus'], decimals: 0)),
                    DetailLine(label: 'Potassium', value: _num(record['potassium'], decimals: 0)),
                    DetailLine(label: 'Humidity', value: record['live_humidity'] == null ? '--' : '${_num(record['live_humidity'], decimals: 0)}%'),
                    DetailLine(label: 'Weather', value: '${record['weather_description'] ?? '--'}'),
                    if (record['land_status'] != null)
                      DetailLine(label: 'Land Status', value: '${record['land_status']}'),
                    const Divider(height: 28),
                    const Text('INFRASTRUCTURE REPORT', style: TextStyle(fontWeight: FontWeight.w900, color: green, letterSpacing: 0.4)),
                    const SizedBox(height: 10),
                    Container(
                      width: double.infinity,
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(
                        color: infrastructureColor(infraData).withOpacity(0.14),
                        borderRadius: BorderRadius.circular(14),
                        border: Border.all(color: infrastructureColor(infraData).withOpacity(0.4)),
                      ),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          if (record['infrastructure_recommendation'] != null) ...[
                            const SizedBox(height: 6),
                            Text(
                    ...
                    DetailLine(label: 'Infrastructure Score', value: '${record['infrastructure_score'] ?? '--'}'),
                    DetailLine(
                      label: 'Risk Level',
                      value: '${record['infrastructure_risk'] ?? record['risk_level'] ?? record['infrastructure_suitability'] ?? '--'}',
                    ),
