...
# for Panabo/Mindanao — exactly the finalized, approved local crop set
# (Section 9): rice, corn, banana, watermelon, papaya, abaca, coconut,
# cacao, durian, cassava, sweet potato, rubber, pomelo. This is not
# rule-based prediction; it only prevents non-local/unapproved classes
# (e.g. mango, mungbean, pigeonpeas, blackgram — previously included here
# by mistake, with no equivalent in the approved crop list) from diluting
# Random Forest training or ever appearing as a learned class.
SUPPORTED_LABELS = {
    'rice', 'maize', 'banana', 'coconut', 'abaca', 'cacao', 'durian',
    'cassava', 'sweet potato', 'rubber', 'pomelo', 'papaya', 'watermelon',
}
...
# ---------------------------------------------------------------------------
# Model comparison: Decision Tree vs SVM vs Random Forest (GeoSustain Section
# 10 requirement). All three are trained and evaluated on the EXACT SAME
# train/test split produced above, so the comparison is apples-to-apples.
# This does NOT change which model is deployed in production — the
# RandomForestClassifier trained above remains the one saved to
# crop_model.pkl and used by the live app. This block only produces the
# comparative accuracy/precision/recall/F1 report for the study, and is
# saved to disk for traceability rather than only printed.
# SVM gets a StandardScaler (standard practice for distance-based models
# with features on very different scales, e.g. N/P/K in the tens-hundreds
# vs pH in single digits) — Decision Tree and Random Forest are scale-
# invariant and use the raw features directly, exactly as before.
# ---------------------------------------------------------------------------
print('\nTraining comparison models (Decision Tree, SVM) on the same split...')

decision_tree = DecisionTreeClassifier(
    max_depth=14, min_samples_leaf=6, min_samples_split=12,
...
svm_model = Pipeline([
    ...
    ('svc', SVC(kernel='rbf', C=10, gamma='scale', class_weight='balanced', random_state=RANDOM_SEED)),
])
...
comparison = {
    ...
    'decision_tree': _metrics(y_test, dt_pred),
    'svm': _metrics(y_test, svm_pred),
...
print('\nModel comparison (same train/test split, same features):')
print(f"{'Model':<15}{'Accuracy':>10}{'Precision':>12}{'Recall':>10}{'F1':>10}")
for name, key in [('Random Forest', 'random_forest'), ('Decision Tree', 'decision_tree'), ('SVM', 'svm')]:
    m = comparison[key]
    print(f"{name:<15}{m['accuracy']*100:>9.2f}%{m['precision_macro']*100:>11.2f}%{m['recall_macro']*100:>9.2f}%{m['f1_macro']*100:>9.2f}%")

COMPARISON_PATH = 'model_comparison_report.json'
import json
with open(COMPARISON_PATH, 'w') as f:
    json.dump(comparison, f, indent=2)
print(f'\nSaved model comparison report: {COMPARISON_PATH}')
print('Random Forest remains the deployed production model (crop_model.pkl unchanged by this comparison).')
